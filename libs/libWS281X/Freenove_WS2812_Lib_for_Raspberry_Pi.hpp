...
public:
	...
	valueType constrain(valueType value, valueType min, valueType max);

	void set_Led_Tpye(unsigned int led_type = WS2811_STRIP_GRB);
	void set_Led_Brightness(rgbIndex brightness = 255);
	void set_Led_Color(unsigned int number, rgbIndex r = 0, rgbIndex g = 0, rgbIndex b = 0);
	void show();
	void clear();
};
#endif
